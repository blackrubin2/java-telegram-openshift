...
import fruitmasterbot.utils.Lang;
...
public class FruitMasterBot extends BotHandler {
	...
	protected boolean onStart(Message message) {
		...
		bot.sendMessage(chatId, lang.START_MESSAGE );
		//genero il menu
		startMenu(message);
	...
	private void startMenu(Message message) {
		...
		bot.sendMessage(""+message.chat().id(), lang.TEXT_MENU_1,ParseMode.Markdown,false,0,rkm);
	}
	...
	private void maintenance(Message message) {
		bot.sendMessage(message.chat().id(),new Lang(message).MAINTENANCE_MESSAGE);
	}
	...
	private String formattaMessaggio(String text, int comboLen) {
		...
		//una volta che sono sicuro che la stringa inizia e finisce con ':' allora posso parsarla
		//		StringBuilder sb = new StringBuilder();
		//		int element = 0;
		//		while(text.length() > 2){
		//			text = text.substring(1);
		//			int endIndex = text.indexOf(':');
		//			String fruit = ':'+text.substring(0,endIndex+1);
		//			System.out.println(fruit);
		//			//controllo che non sia un frutto a cui sono allergico
		//			boolean flag = false;
		//			for(int i=0;i<FRUTTI.length;i++) 
		//				if(fruit.equals(FRUTTI[i])){
		//					flag = true;
		//				}
		//			if(!flag) return null;
		//			//frutto buono, lo metto nel cestino
		//			sb.append(fruit);
		//			if(element <4) sb.append('|');
		//			text = text.substring(endIndex+1);
		//			if(text.length()>0 && text.charAt(0) != ':') return null; 
		//			element++;
		//		}
		//		if(element != 4) return null;
		//		return sb.toString();
	...
	private void sorry(Message message) {
		bot.sendMessage(message.chat().id(), new Lang(message).SORRY_MESSAGE);
	}

	private void playSingle1(Message message, boolean debug, String args) {
		Lang lang = new Lang(message);
		String[] arg = args.split(" ");
		bot.sendMessage(""+message.chat().id(), lang.GENERATE_FRUIT_MIX_MESSAGE);
		//---genero combinazione---//
		int comboLen = 4;
...
//			rsGame.close();
			...
			bot.sendMessage(""+message.chat().id(), lang.END_FRUIT_MIX_P1_MESSAGE+tentativi+lang.END_FRUIT_MIX_P2_MESSAGE);
	...
	private void help(Message message) {
		...
		bot.sendMessage(message.chat().id(), new Lang(message).WIP_MESSAGE);
	}
	...
	private void error(Message message, Exception e) {
		...
		bot.sendMessage(message.chat().id(), "" + e.getClass() + ':' + e.getMessage() + '\n' + sb.toString());
